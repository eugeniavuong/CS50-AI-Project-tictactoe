...
def result(board, action):
    ...
    """
    # Create the board within modifying the original board received as input 
...
def max_value(board):
    ...
    for action in actions(board):
        newmin, act = min_value(result(board, action))
        if newmin > v:
            v = newmin
            move = action
            if v == 1:
...
def min_value(board):
    ...
    move = None
    for action in actions(board):
        newmax, act = max_value(result(board,action))
        if newmax < v:
            v = newmax 
            move = action
            if v == -1:
